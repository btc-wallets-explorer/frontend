const generateUTXOLinks = (unspentMap, transactionMap, scriptHashesMap) =>
  Object.values(unspentMap).flatMap((u) =>
    u.utxos.map((utxo) => ({
      type: "utxo",
...
const generateTXOs = (transactionMap, scriptHashesMap) => {
  const histories = Object.values(scriptHashesMap).flatMap((v) =>
    v.transactions.map((hist) => ({
      txid: hist.tx_hash,
      ...v,
    })),
  );

  const incomingTxos = histories
    .flatMap((h) =>
      transactionMap[h.txid].vin
        ...
        .map((vin) => ({
          ...h,
          vin,
          vout: transactionMap[vin.txid].vout[vin.vout],
        })),
    )
...
const generateTXONodes = (transactionMap) =>
  Object.values(transactionMap).map((tx) => ({
    type: "txo",
