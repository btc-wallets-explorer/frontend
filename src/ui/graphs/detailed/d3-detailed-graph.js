...
import { createNetwork } from "./detailed-model";
...
export const d3ForceGraph = (root, store, blockchain, settings) => {
  ...
  ["collide", "charge", "link", "x", "y"].forEach((force) =>
    observe(`ui.forceStrength.${force}`, (data) => {
  ...
  const drawTransactionDetails = (items) => {
    ...
    items.append((node) => {
      ...
      if (node.tx && node.tx.vin.length < 11) {
        const calcVinY = (tx, vin) =>
          (nodeHeight / tx.vin.length) * (tx.vin.indexOf(vin) + 0.5);
        ...
        group.appendChild(
          createLine("vin many", -WIDTH, nodeHeight / 2, 0, nodeHeight / 2),
        );
      }

      if (node.tx && node.tx.vout.length < 11) {
        const calcVoutY = (tx, vout) =>
          (nodeHeight / tx.vout.length) * (tx.vout.indexOf(vout) + 0.5);
  ...
  const link = svg
    ...
    .append("path")
    .attr("stroke-width", (d) =>
      d.type === "txo" ? d.value * 30 : (d.value * 30) / 100000000,
    )
  ...
  const calcVoutYOffset = (d) =>
    d.source.type === "txo"
      ? (nodeHeight / d.source.tx.vout.length) *
        (d.source.tx.vout.indexOf(d.vout) + 0.5)
      : nodeHeight / 2;

  const calcVinYOffset = (d) =>
    d.target.type === "txo"
      ? (nodeHeight / d.target.tx.vin.length) *
        (d.target.tx.vin.indexOf(d.vin) + 0.5)
      : nodeHeight / 2;
  ...
  simulation.on("tick", () => {
    ...
    d3.selectAll(queryAll(".node_block")).attr(
      "transform",
      (d) => `translate(${d.x},${d.y})`,
    );
  });
  ...
  observe("ui.selections", (data) => {
    ...
    d3.selectAll(queryAll(".node_rect")).attr(
      "class",
      (n) => `node_rect ${txIds.includes(n.tx.txid) ? "selected" : ""}`,
    );
  });
